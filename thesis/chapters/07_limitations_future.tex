...
exponential signal-decay problem \citep{goemaere2025epc}---is avoided at our depth
rather than addressed. Our networks are shallow enough that signal decay does not
dominate, and while the general-depth kernel validates correctness up to five layers,
correctness is not the same as trainability. We neither reproduce the depth-scaling
failure systematically nor evaluate the proposed fixes (error optimisation,
$\mu$P-style parametrisation, forward-anchored gradient corrections of
\citet{qi2025settling}). The problem is acknowledged and deferred, not resolved.
...
Finally, although solving the depth-scaling problem is frontier work, a small project
can \emph{characterise} it precisely. The concrete plan is to reproduce the
``deeper-is-worse'' effect on MNIST across two, four, and eight hidden layers, measure
and plot the per-layer energy and gradient distribution to expose the signal-decay
imbalance, and then reimplement one proposed fix---error optimisation
\citep{goemaere2025epc} or a $\mu$P-style parametrisation---on the small network to
show whether it flattens the imbalance. This would convert the depth-scaling
limitation from an acknowledged gap into a clean empirical study, and it is the
single most valuable scientific extension that remains within reach of the present
codebase.
